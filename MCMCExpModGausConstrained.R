...
# Solving for s
mcmc.erfcx_eq2 <- function(s,l,mode,c){
    if (rightTail){
        erfcx((1/sqrt(2))*((s*l) - ((mode - c)/s))) - (sqrt(2/pi) / (s*l))
    } else {
        erfcx((1/sqrt(2))*((s*l) - ((c - mode)/s))) - (sqrt(2/pi) / (s*l))
    }
}
...
mcmc.erfcxinvroot2 <- function(l,mode,c){
    uniroot(mcmc.erfcx_eq2, lower = 1, upper = 20, l=l, mode=mode, c=c)$root
}
...
#g.sig <- rep(5,npeaks)
#g.lamb <- rep(0.35,npeaks) # lambda of 1 means nearly gaussian. Closer to 0 means a broader tail

cat("\nGuess the EMG means! (Left of peaks if tail is on left)\n\n")
guessemgmean <- locator(npeaks)

# EMG sample mean and sd can be calculated with spectrum data - but for a double peak this will be impossible

cat("\nGuess the Gaussian component means! (Right of peaks is tail is on left)\n\n")
guessgaussmean <- locator(npeaks)
g.cent <- guessgaussmean$x

lambfun <- function(emgmean, gaussmean){1/abs(gaussmean - emgmean)}
g.lamb <- mapply(lambfun, guessemgmean$x, g.cent)

erfcxinvroot2_guess <- mapply(mcmc.erfcxinvroot2, g.lamb, guess$x, g.cent)
g.sig <- erfcxinvroot2_guess
...
mod <- quap(
    alist(
        ...
        intens ~ dnorm(g.intens,5),
        cent ~ dnorm(g.cent,1),
        sig ~ dlnorm(g.sig, 0.1), # 2
        lamb ~ dnorm(g.lamb,0.1), # 0.2
        bg.intens ~ dnorm(g.bg.intens,0.1),
        bg.slope ~ dnorm(0,0.01)
    ),
    data=d,
    start=list(bg.intens=g.bg.intens,bg.slope=g.bg.slope,
